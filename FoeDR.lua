...
function FoeDR.new(strDRType)
	...
	local self = setmetatable(
		...
		DRType = strDRType,
		_DRLevel = 0,
		_TimeLastApplied = GetPvPClockTime(),
		_Expires = GetPvPClockTime(),
		_DRExpires = 0
		}, FoeDR)	
	 -- return the instance
	return self
...
function FoeDR:ApplyDR(duration)
	if self._DRLevel < 3 then
			...
			self._TimeLastApplied = GetPvPClockTime();
			self._Expires = GetPvPClockTime() + CONSTANTS.DRTIME + duration;
	end
...
function FoeDR:ResetDR()
	...
	self._TimeLastApplied = GetPvPClockTime();
	self._Expires = GetPvPClockTime() + CONSTANTS.DRTIME;
...
--  --print("DEBUG:FoeDR:_CalculateDR(), "..tostring(self._Expires));
	if (GetPvPClockTime() >= self._Expires) then
			self._DRLevel = 0;
	end
...
function FoeDR:DRExpires()
	local clocktime = GetPvPClockTime();
	local expSeconds = 0
